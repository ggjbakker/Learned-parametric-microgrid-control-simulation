function [f1,f2,f3,f4,f5,f6,f7,f8,f9,xs] = conPar_fs_gen1(x,Prob)
u0 = Prob.user.u0;
x0 = Prob.user.x0;
...
f1(1) = x0/xub;
f2(1) = x0*csale(1)*uub(4)*(uub(2)+cbuy(1)*cprod(1))/(xub*uub(4)*uub(2)*csale(1));
f3(1) = x0*csale(1)/(xub*csale(1));
...
for i = 1:N-1
    ...
    f1(i+1) = xs/xub;
    f2(i+1) = xs*csale(i+1)*uub(4)*(uub(2)+cbuy(i+1)*cprod(i+1))/(xub*uub(4)*uub(2)*csale(1));
    f3(i+1) = xs*csale(i+1)/(xub*csale(1));
