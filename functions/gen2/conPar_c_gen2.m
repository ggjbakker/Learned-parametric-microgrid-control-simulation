function c = conPar_c_gen2(x,Prob)
global US_B
u0 = Prob.user.u0;
x0 = Prob.user.x0;
...
US_B.f1(1) = ulb(3)-csale(1)+2*x0;
US_B.f2(1) = csale0*x0*cbuy0^2*(cbuy(1)-1);
US_B.f3(1) = csale(1)^2*x0*(1+uub(4));

US_B.f4(1) = csale(1);
US_B.f5(1) = csale(1)*cprod(1)-2*cbuy(1);
US_B.f6(1) = cprod(1);

% US_B.f7(1) = cprod(1);
% US_B.f8(1) = csale(1);
% US_B.f9(1) = (Pload0+Pres0+u0(2)+u0(3))/1500;
...
%u4 = US_B.f7(1)*x(7) + US_B.f8(1)*x(8) + x(9)*US_B.f9(1)+x(10);
...
for i = 1:N-1
    ...
    US_B.f1(i+1) = ulb(3)-csale(i+1)+2*xs;
    US_B.f2(i+1) = csale(i)*xs*cbuy(i)^2*(cbuy(i+1)-1);
    US_B.f3(i+1) = csale(i+1)^2*xs*(1+uub(4));

    US_B.f4(i+1) =  csale(i+1);
    US_B.f5(i+1) = csale(i+1)*cprod(i+1)-2*cbuy(i+1);
    US_B.f6(i+1) = cprod(i+1);

%     US_B.f7(i+1) = cprod(i+1);
%     US_B.f8(i+1) = csale(i+1);
%     US_B.f9(i+1) = (Pload(i)+Pres(i)+u2+u3)/1500;
...
%     u4 = US_B.f7(i+1)*x(7) + US_B.f8(i+1)*x(8) + x(9)*US_B.f9(i+1)+x(10);
